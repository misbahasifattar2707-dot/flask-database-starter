...
Part 4: REST API with Flask
===========================
Complete REST API with Author & Book relationship
"""
...
class Book(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'isbn': self.isbn,
            'author': {
                'id': self.author_obj.id,
                'name': self.author_obj.name
            },
...
@app.route('/')
def index():
    return "<h1>REST API with Author & Book</h1><p>Use Postman or curl</p>"
...
def init_db():
    with app.app_context():
        ...
        if Author.query.count() == 0:
            a1 = Author(name='Eric Matthes', city='USA')
            a2 = Author(name='Miguel Grinberg', city='Spain')

            db.session.add_all([a1, a2])
            db.session.commit()

            b1 = Book(title='Python Crash Course', year=2019, author_id=a1.id)
            b2 = Book(title='Flask Web Development', year=2018, author_id=a2.id)
